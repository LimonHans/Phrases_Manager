...
#################################################
#This 'Explained_Code.py' contains explanations.#
#################################################

def text_adder(fir, sec, mark):
    # This void is used to add one phrase.
    # 'fir' refers to the words being learned, 'sec' refers to translation and 'mark' will generate a file contaning only the same marked words
    if fir == "" or sec == "":
        print("No Words Detected")
        return
    sentence = fir + ", " + sec# make a combination
    sentence += "\n"
    if mark != "":
    ...
    return
def Phrases_Adder():
    # The void is used to add phrases.
    Adder_Window = Tk(baseName = "1", className = "1")
    # It seems that if the windows have no 'baseName' and 'className' some errors may occur.
    Adder_Window.title("Phrases Adder")
    Adder_Window.resizable(False, False)
    Adder_Window.geometry("+620+410")

    Label(Adder_Window, text = "短语").grid(row = 0)# ask for the phrase being learned
    Label(Adder_Window, text = "释义").grid(row = 1)# ask for its translation
    Label(Adder_Window, text = "特殊标记").grid(row = 2)# ask for a mark. If it is blank, then that means "No makrs".
    ...
    def passer(func):
        # This one can call 'text_adder'.
        # 'func' is a variable only controlled by the following 2 buttons.
        text_us = entry1.get()
        text_zh = entry2.get()
        mark = entry3.get()
    ...
    return
def Files_finder():
    # This void is used to list out the files
    ans = []
...
def Text_finder(place):
    # This void can give back a random phrase in a particular file
    ans = f_cache.getlines(place)
    return ans[rand.randint(0, ans.__len__() - 1)].split(", ", 1)
def Phrases_Tester():
    # The void is used to test yourself
    list = Files_finder()
    ...
    # A list contains 'state', 'aim', 'translation', '*translation', 'button text'.
    # 'state' tells whether to show the translation or move on to the next phrase.
    # 'aim' means the words being learned
    ...
    def FollowText(alpha):
        if alpha[0] == 0:# show the phrase without translation
            ...
            alpha[3] = Zhtext
            alpha[4].set("展示释义")
        elif alpha[0] == 1:# show the translation
            alpha[2].set(alpha[3])
            alpha[4].set("下一组短语")
        alpha[0] = (alpha[0] + 1)%2
...
def Sort_phrases(place):
    # Sort phrases in a particular file
...
def Shuffle_phrases(place):
    # Shuffle phrases in a particular file
    goal = f_cache.getlines(place)
    rand.shuffle(goal)
    f = open(place, "w", encoding = "utf-8")
    for sentence in goal: f.write(sentence)
...
def Asker(Title, Question):
    ...
    # NOTICE: Do Not Try To Replace It With Messagebox, OR Errors Will Occur!
    ans = [False]
...
def Noticer(Title, Result):
    ...
    return
def Exporter(mode):
    # Export phrases into a single '.docx' file.
    # 'mode' is used to decide sort or not.
