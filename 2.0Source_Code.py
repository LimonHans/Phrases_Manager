...
def Phrases_Tester():
    list = Files_finder()
    for files in list: Shuffle_phrases(files)
    ...
    alpha = [0, StringVar(), StringVar(), "", StringVar(), 0, 0, False]
    alpha[4].set("开始测试")
    words = []

    def FollowText(alpha, _words_, _list_):
        ...
        if alpha[0] == 0:
            ...
            if alpha[5] >= _words_.__len__():
                alpha[5] = 0
                _words_ = f_cache.getlines(_list_[alpha[6]])
                alpha[6] += 1
            print(_words_)
            print(_list_)
            Ustext, Zhtext = _words_[alpha[5]].split(", ", 1)
            ...
            alpha[2].set(alpha[3])
            if alpha[6] >= _list_.__len__():
                alpha[7] = True
                alpha[4].set("测试完毕，退出")
            else: alpha[4].set("下一组短语")
        alpha[0] = (alpha[0] + 1)%2
    ...
    Changer = Button(Tester_Window, textvariable = alpha[4], width = 20, height = 4, command = lambda: FollowText(alpha, words, list))
